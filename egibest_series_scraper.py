...
async def scrape_egibest_series(max_pages=None):
    ...
    try:
        async with async_playwright() as p:
            ...
            # 2. منع الصور لتسريع العملية وتقليل استهلاك الرام (تحسين أداء)
            await page.route(
            ...
            while True:
                ...
                try:
                    ...
                    if response.status == 404:
                        print(
                            f"🏁 تم الوصول لنهاية الصفحات عند الصفحة {current_page - 1}"
                        )
                        break

                    await asyncio.sleep(2)  # انتظار تحميل العناصر الديناميكية

                    # السيلكتور بناءً على بنية الموقع الحالية
                    items = await page.query_selector_all("a.postBlockCol")

                    if not items:
                        print(
                            f"🛑 لا توجد عناصر إضافية. انتهى السحب عند صفحة {current_page}"
                        )
                        break
